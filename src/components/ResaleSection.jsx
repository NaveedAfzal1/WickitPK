...
export default function ResaleSection({ listings, wallet, onBuyResale }) {
    ...
    <div style={{ marginTop: 48 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
        {listings.map(l => {
          ...
          const savings = ((l.listPrice / l.faceValue - 1) * 100).toFixed(0);
          return (
            <div key={l.tokenId} style={{ ...S.ticketCard, padding: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: 12 }}>
                <div>
                  <span style={S.ticketBadge(l.tier)}>{l.tier}</span>
                  <h3 style={{ fontFamily: font, fontSize: 16, fontWeight: 700, marginTop: 8 }}>{match?.home} vs {match?.away}</h3>
                  <p style={{ fontSize: 12, color: COLORS.gray }}>{match?.date} &middot; Token #{l.tokenId}</p>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontFamily: font, fontSize: 20, fontWeight: 800, color: COLORS.gold }}>PKR {l.listPrice.toLocaleString()}</div>
                  <div style={{ fontSize: 11, color: COLORS.grayDark }}>Face: PKR {l.faceValue.toLocaleString()}</div>
              ...
              <div style={{ marginTop: 8, fontSize: 11, color: COLORS.grayDark }}>Seller: {l.seller} &middot; 5% royalty to organizer</div>
            </div>
          );
        })}
      </div>
